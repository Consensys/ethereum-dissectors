...
static int dissect_devp2p_wire_udp(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree _U_, void *data _U_) {
	...
	if (tvb_get_guint8(tvb, 0) == '?' && tvb_get_guint8(tvb, 1) == '?' &&
		...
		if (!PINFO_FD_VISITED(pinfo)) {
			...
			temp_data[0] = (peer_ip >> 24) & 0x000000FF;
			temp_data[1] = (peer_ip >> 16) & 0x000000FF;
			temp_data[2] = (peer_ip >> 8) & 0x000000FF;
			temp_data[3] = (peer_ip) & 0x000000FF;
			dst.data = (const void *)temp_data;
			dst.priv = NULL;
			guint32 my_ip = 0;
			...
			if (!secret_in && !secret_out) {
				...
				secret_in->start = TRUE;
				secret_out->start = FALSE;
